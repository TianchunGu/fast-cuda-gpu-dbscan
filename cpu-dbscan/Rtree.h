...
// Rtree.h 文件在项目中主要提供了 R 树（R-tree） 数据结构的实现。
// R树是一种多维空间索引结构，它能够高效地存储和查询多维数据（例如二维或三维空间中的点或矩形）。
// R 树的实现在 CPU 版本的 DBSCAN 算法中被用于构建空间索引，以优化其性能。
// Rtree.h 提供了 DBSCAN 算法中用于快速查找点邻居的基础空间数据结构。

// NOTE This file compiles under MSVC 6 SP5 and MSVC .Net 2003 it may not work
// on other compilers without modification. 注意：此文件可在 MSVC 6 SP5 和 MSVC
// .Net 2003 环境下编译成功，但若使用其他编译器则可能需要进行修改才能正常运行。

// NOTE These next few lines may be win32 specific, you may need to modify them
// to compile on other platform 注意：接下来的这几行代码可能与 Windows 32
// 位系统相关，您可能需要对其进行修改才能在其他平台上编译。
#include <assert.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
class RTree {
 ...
 protected:
  ...
  bool InsertRectRec(const Branch& a_branch,
                     Node* a_node,
                     Node** a_newNode,
                     int a_level);
...
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRec(const Branch& a_branch,
                               Node* a_node,
                               Node** a_newNode,
                               int a_level) {
...
//
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRect(const Branch& a_branch,
                            Node** a_root,
                            int a_level) {
  ASSERT(a_root);
...
RTREE_TEMPLATE
bool RTREE_QUAL::AddBranch(const Branch* a_branch,
                           Node* a_node,
                           Node** a_newNode) {
  ASSERT(a_branch);
  ASSERT(a_node);
